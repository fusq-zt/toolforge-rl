...
- 设计 correctness-gated group-relative efficiency reward，在相同初始化与 rollout 预算下，相比 Vanilla GRPO 将 Calls per Correct 改变 **+21.84%**，答案准确率变化 **-0.04 pp**；实验结论按冻结成功标准判定为 **PASS**。
...
- Designed a correctness-gated group-relative efficiency reward; under identical initialization and rollout budgets, changed calls per correct by **+21.84%** with **-0.04 pp** accuracy change versus Vanilla GRPO. The frozen success criteria evaluate the outcome as **PASS**.
